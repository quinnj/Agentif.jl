...
@assert occursin("Test 1 PASS", message_text(result.message)) "Test 1 failed - echo command didn't work"
println("✅ Test 1 passed")

# Test 2: Working directory
println("\n[2/3] Testing working directory...")
result = evaluate(agent, "Run 'pwd' in the /tmp directory")
@assert occursin("tmp", lowercase(message_text(result.message))) "Test 2 failed - working directory not respected"
println("✅ Test 2 passed")

# Test 3: Multiple commands
println("\n[3/3] Testing multiple commands...")
result = evaluate(agent, "Run these 3 commands: echo 'First', echo 'Second', echo 'Third'")
@assert occursin("First", message_text(result.message)) || occursin("Second", message_text(result.message)) "Test 3 failed - multiple commands didn't work"
