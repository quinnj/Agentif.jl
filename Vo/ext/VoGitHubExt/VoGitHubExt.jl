...
struct GitHubWebhookEvent <: Vo.Event
    kind::String
    action::String
...
const ALL_EVENT_TYPES = [Vo.EventType("github_$k", "GitHub $k webhook event") for k in GITHUB_WEBHOOK_KINDS]

# ─── EventSource ───

Base.@kwdef mutable struct GitHubEventSource <: Vo.EventSource
...
function _get_installation_auth(source::GitHubEventSource, payload::AbstractDict)
    ...
    try
        ...
        @warn "VoGitHubExt: failed to create installation access token" exception=(e, catch_backtrace())
        return nothing
    end
end

Vo.get_event_types(::GitHubEventSource) = ALL_EVENT_TYPES

function Vo.start!(source::GitHubEventSource, assistant::Vo.AgentAssistant)
    ...
    errormonitor(Threads.@spawn begin
        listener = GitHub.EventListener(;
            ...
            @info "VoGitHubExt: event" kind action repo=repo_name sender=sender_login name=Vo.get_name(ghev)
            put!(assistant.event_queue, ghev)
            return HTTP.Response(200, "OK")
        end
        @info "VoGitHubExt: listening on $(source.host):$(source.port)"
        Base.run(listener, host, port)
    end)
end

end # module VoGitHubExt
