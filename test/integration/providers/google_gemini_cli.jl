...
@testset "Google Gemini CLI Stream" begin
    apikey = get(() -> nothing, ENV, "GOOGLE_GEMINI_CLI_API_KEY")
    model_id = get(() -> nothing, ENV, "GOOGLE_GEMINI_CLI_MODEL")
    provider = get(() -> "google-gemini-cli", ENV, "GOOGLE_GEMINI_CLI_PROVIDER")
    if apikey === nothing || model_id === nothing
        @info "Skipping Google Gemini CLI stream; set GOOGLE_GEMINI_CLI_API_KEY (JSON with token/projectId) and GOOGLE_GEMINI_CLI_MODEL (optional GOOGLE_GEMINI_CLI_PROVIDER)"
        return
    end

    model = Agentif.getModel(provider, model_id)
    model === nothing && error("unknown model: provider=$(repr(provider)) model_id=$(repr(model_id))")
    model.api != "google-gemini-cli" && error("expected google-gemini-cli model, got api=$(repr(model.api))")
